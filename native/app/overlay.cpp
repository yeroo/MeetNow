...
void Overlay::update(const std::vector<Meeting>& meetings) {
    ...
    // (m.Start > now && m.Start <= now.AddHours(2)). At start time the
    // meeting moves to the bottom-right join popup (popup.cpp), so started
    // meetings deliberately leave this badge; the tray keeps them until end.
